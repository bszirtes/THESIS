...
usage() {
    echo "Usage: $0 <nodes> <num_servers> <num_clients> <num_messages> <prime_range> [--no-load-balancer] [--verbose] [--kubeconfig /path/to/kubeconfig]"
...
SERVER_IMAGE="localhost:5001/ping-pong-lb-prime-server:v1"
CLIENT_IMAGE="localhost:5001/ping-pong-lb-prime-client:v1"
...
if [ -z "$NUM_SERVERS" ] || [ -z "$NUM_CLIENTS" ] || [ -z "$NUM_MESSAGES" ] || [ -z "$PRIME_RANGE" ] || [ -z "$KUBECONFIG" ]; then
    usage
fi
